    ...
    try:
        ...
        if recaptcha_secret and recaptcha_token:
            ...
            if not rc_resp.get("success") or rc_resp.get("score", 0) < 0.5:
                return jsonify({"erro": "Atividade suspeita detectada (Falha no reCAPTCHA)"}), 403
        ...
        # Gera JWT
        access_token = create_access_token(identity=usuario['id'], expires_delta=timedelta(hours=8))
...
def criar_usuario():
    try:
        ...
        resp_check = supabase.table('usuarios').select('id').eq('email', email).execute()
